...
- `channels.digirig.rx.energyThreshold` and `channels.digirig.rx.carrierSenseThreshold`: A two-tier VAD system. `energyThreshold` (e.g. 0.1) triggers the recording when the user speaks. `carrierSenseThreshold` (e.g. 0.0001) keeps the channel busy and recording alive as long as the radio's squelch is open (even if the user pauses).
