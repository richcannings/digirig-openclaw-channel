...
export default function register(api: {
    ...
    api.registerTool({
        ...
        label: "DigiRig TX",
        description:
            "Transmit text over DigiRig (respects tx.policy=proactive).",
        parameters: Type.Object({
            text: Type.String({ description: "Text to transmit over DigiRig" }),
            callsign: Type.Optional(
                Type.String({ description: "Override callsign" }),
            ),
        }),
        async execute(
            _toolCallId: string,
            params: { text?: string; callsign?: string },
        ) {
            ...
            if (!allowToolTx) {
                throw new Error(
                    "digirig_tx is disabled by tx.allowToolTx=false",
                );
            }
            const callsign = (
                params?.callsign ??
                cfg.channels?.digirig?.tx?.callsign ??
                DEFAULT_TX_CALLSIGN
            ).trim();
            const runtime = getRuntime();
            await runtime.speak(appendCallsign(text, callsign));
